...
from nicky.modules.helper_funcs.string_handling import markdown_parser
from nicky.modules.disable import DisableAbleCommandHandler

import nicky.modules.sql.feds_sql as sql

from nicky.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "nicky_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="nicky_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "nicky_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="nicky_fbanned_users.csv",
