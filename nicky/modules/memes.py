...
from nicky.modules.helper_funcs.extraction import extract_user
...
from nicky import dispatcher, SUDO_USERS, SUPPORT_USERS, LOGGER
from nicky.modules.disable import (
    DisableAbleCommandHandler,
    DisableAbleMessageHandler,
)

import nicky.modules.helper_funcs.fun_strings as fun
...
`/decide` can be also used with regex like: `nicky? <question>: randomly answer "Yes, No" etc.`

Some other regex filters are:
`me too` | `goodmorning` | `goodnight`.

nicky will reply random strings accordingly when these words are used!
...
DECIDE_HANDLER = DisableAbleMessageHandler(
    Filters.regex(r"(?i)^nicky\?"), decide, friendly="decide"
)
