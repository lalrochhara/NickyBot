...
from nicky.modules.helper_funcs.admin_rights import (
    user_can_pin,
    user_can_promote,
    user_can_changeinfo,
)
...
def promote(update, context):
    ...
    if not user_id:
        message.reply_text("Pakhat tal mention rawh.......")
        return ""

    user_member = chat.get_member(user_id)
    if (
        user_member.status == "administrator"
        or user_member.status == "siamtu"
    ):
...
def demote(update, context):
    ...
    if user_member.status == "creator":
        message.reply_text("He group siamtu hi ka demote nguar ang.... ")
        return ""
...
def adminlist(update, context):
    ...
    text = "He group <b>{}</b> lama Admin te:".format(
