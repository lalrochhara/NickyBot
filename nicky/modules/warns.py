...
from nicky import dispatcher  # BAN_STICKER
from nicky.modules.disable import DisableAbleCommandHandler
from nicky.modules.helper_funcs.chat_status import (
    is_user_admin,
    bot_admin,
    user_admin_no_reply,
    user_admin,
...
from nicky.modules.helper_funcs.extraction import (
    extract_text,
    extract_user_and_text,
    extract_user,
)
from nicky.modules.helper_funcs.filters import CustomFilters
from nicky.modules.helper_funcs.misc import split_message
from nicky.modules.helper_funcs.string_handling import split_quotes
from nicky.modules.helper_funcs.alternate import typing_action
from nicky.modules.log_channel import loggable
from nicky.modules.sql import warns_sql as sql
...
def warn(
    ...
    if num_warns >= limit:
        ...
        log_reason = (
            ...
            "\n<b>User:</b> {} (<code>{}</code>)"
            "\n<b>A chhan:</b> {}"
            "\n<b>Hmuhtawh zat:</b> <code>{}/{}</code>".format(
        ...
        reply = "User {} hian warning {}/{} lai a hmu tawh... fimkhur hle rawh!".format(
            mention_html(user.id, user.first_name), num_warns, limit
        )
        if reason:
            reply += "\nWarning dawn leh chhan thar ber:\n{}".format(html.escape(reason))
        ...
        log_reason = (
            ...
            "\n<b>User:</b> {} (<code>{}</code>)"
            "\n<b>A chhan:</b> {}"
            "\n<b>Hmuh tawh zat:</b> <code>{}/{}</code>".format(
                html.escape(chat.title),
                warner_tag,
                mention_html(user.id, user.first_name),
                user.id,
    ...
    try:
        ...
        if excp.message == "I Message reply na khi hmuh anilo":
            # Do not reply
            message.reply_text(
                reply,
    ...
    if match:
        ...
        if res:
            update.effective_message.edit_text(
                "Warning thar ber paih tu {}.".format(
                    mention_html(user.id, user.first_name)
                ),
                parse_mode=ParseMode.HTML,
            )
...
def warns(update, context):
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            text = "He user hian warning {}/{} lai a tawk tawh, a tawh chhan tlangpui te chu:".format(
                num_warns, limit
            )
